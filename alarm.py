...
#     # functin for set an alarm
# def set_alarm():
#     test = False
#     while not test:
#         try:
#             clock_alarm = (int(input("Set the alarm!\n\nFirst, choose hours : ", )), int(input("Next, choose minutes : ", )),
#                      int(input("And at the end, choose seconds : ", )))
#             if 0 <= clock_alarm[0] < 24:
#                 if 0 <= clock_alarm[1] < 60:
#                     if 0 <= clock_alarm[2] < 60:
#                         return clock_alarm
#                     else:
#                         print("Seconds has to be in a correct range!")
#                 else:
...
#     alarm_sec = clock_alarm[0] * 3600 + clock_alarm[1] * 60 + clock_alarm[2]
#     alarm_sec = alarm_sec - 3600  # Correct time because it begins at 1:00 AM
#     return alarm_sec
...
def user_time():
    ...
    while True:
        if alarm_sec == clock_sec :
            print ("It'a time to wake up and run!")
        else :
            print("\r", time.strftime("%H:%M:%S", time.localtime(clock_sec)), end="")
            clock_sec += 1
            time.sleep(1)
